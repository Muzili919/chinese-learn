...
// ─── 小学数学（小升初选拔体系·对标猿辅导/作业帮）─────────
const MATH_EXAM_TYPE_MAP = { xsc: '小升初模拟', unit: '单元综合', final: '期末综合', review: '总复习' }

// 6大核心应用模型（小升初必考）
const MATH_MODELS = [
  { id: 'xingcheng', name: '行程问题', emoji: '🏃', core: '路程=速度×时间', variants: ['相遇问题','追及问题','火车过桥','流水行船','环形跑道'] },
  { id: 'gongcheng', name: '工程问题', emoji: '🏗️', core: '工作效率=工作量÷时间', variants: ['合作工程','交替工作','水管注水','进水排水'] },
  { id: 'jitutonglong', name: '鸡兔同笼', emoji: '🐔', core: '假设法/方程法', variants: ['鸡兔同笼','租车租船','得失比较'] },
  { id: 'nianling', name: '年龄问题', emoji: '⏰', core: '年龄差不变', variants: ['和倍问题','差倍问题','年龄推算'] },
  { id: 'yingkui', name: '盈亏问题', emoji: '📊', core: '分配方案对比', variants: ['盈亏问题','方案选择','最优配置'] },
  { id: 'bili', name: '比例分配', emoji: '📐', core: '按比例分配总量', vars: ['正比例分配','反比例应用','浓度配比'] },
]

function getMathExamPrompt(examType) {
  const examLabel = MATH_EXAM_TYPE_MAP[examType] || '小升初模拟'
  return `你是拥有15年一线教学经验的"小升初数学命题专家"，曾在某知名教育机构（类似猿辅导/作业帮）担任小学数学学科负责人。你出的试卷被多所重点初中用于分班考试参考。

【出卷任务】
为六年级学生出一份"${examLabel}"数学试卷。这不是普通期末测验，而是**小升初选拔级别的综合能力测试卷**。
满分100分，难度系数0.65-0.72（即中等偏上，区分度强）。
...
1. 🏃 行程问题（核心公式：S=vt）
   · 相遇：S=(V1+V2)×t  |  追及：S=(V大-V小)×t
   · 必须有生活化场景（高铁/快递/跑步），禁止"甲乙两地相距..."

2. 🏗️ 工程问题（核心：工作效率）
   · 合作：总效率=各效率之和  |  公式：1÷(1/t1 + 1/t2)
   · 考虑"实际场景"：装修队/打印文件/做蛋糕

3. 🐔 鸡兔同笼（核心：假设法/方程法）
   · 不只出"鸡兔"，可以变形为：租车/买票/得分
   · 要求学生能识别出"这是鸡兔同笼模型"
...
一、【计算与运算】4道 × 6分 = 24分（🟢基础层）

  第1题 — 四则混合运算（含分数+百分数+括号）
    示例：(3/4 + 2/5) × 20 - 125% × 8
    要求：考查运算顺序和分数小数互化能力
    
  第2题 — 简便运算（巧用运算律）
    示例：99 × 37 + 37  或  3.7 × 9.9 + 0.37
    要求：必须有技巧空间（凑整/提取公因数/乘法分配律）
    
  第3题 — 解方程或解比例（含分数系数）
    示例：① (2/3)x + 1/4 = 5/6   ② x : 3.5 = 4 : 7
    要求：考查等式性质和分数运算
    
  第4题 — 列式计算（文字描述→综合算式）
    示例："一个数的40%比它的1/4多15，求这个数"
    要求：翻译文字为数学表达式的能力

二、【选择题】6道 × 4分 = 24分（🟢🟡混合）

  ★ 每道选择题的4个选项(A/B/C/D)必须精心设计：
  - A选项：正确答案
  - B选项：典型错误（如单位漏换、公式记反、条件看错）
  - C选项：干扰陷阱（如计算粗心错误、概念混淆）
  - D选项：完全无关或方向性错误
  
  分布要求：
  · 数与代数 2题（百分数应用/比例判断/正反比例辨析）
  · 图形与几何 2题（组合图形面积/立体图形展开/几何变换）
  · 统计与概率 1题（平均数vs中位数/可能性计算/抽样理解）
  · 奥数思维 1题（数论余质/逻辑推理/最值问题）

三、【填空题】4道 × 4分 = 16分（🟢🟡混合）

  · 第1题：单位换算进率（长度/面积/体积/时间复合换算）
    如：2.5小时=( )分钟  |  3050mL=( )L=( )mL
  · 第2题：比和比例的性质运用
    如：a:b=3:5，则(a+10):(b+10)=?  或  若x:y=2:3且y=15则x=?
  · 第3题：规律探究（数列找规律/图形计数/周期问题）
  · 第4题：概念理解填空（折扣率/利率/概率定义）

四、【操作与应用】2道 × 9分 = 18分（🟡应用层·核心）

  这两道是全卷最重要的"模型识别+解题"题：
  
  第1题（9分）— 几何操作题
    给出组合图形（用文字精确描述图形构成和数据），
    要求学生：
    ① 识别图形类型和已知条件（2分）
    ② 列出正确的计算公式（3分）
    ③ 写出完整计算过程（3分）
    ④ 给出最终答案和单位（1分）
    图形类型优先：阴影面积割补法 / 立体图形展开图 / 
              圆环/扇形面积 / 组合体体积
...
五、【解决问题】2道 × 9分 = 18分（🟡🔴综合层·压轴）

  这两道决定学生能否进入重点班：
  
  第1题（9分）— 经典模型变式（从6大模型中选一个）
    要求：
    - 生活化场景（不是"水池注水""鸡兔同笼"老套路）
    - 至少2步推理
    - 答案格式：设未知数→列方程/算式→求解→答
    - 必须在analysis中标明属于哪个模型
    
  第2题（9分）— 综合创新题（跨知识点融合）
    要求：
    - 同时涉及2个以上知识领域（如：行程+比例 / 几何+分数）
    - 有一定思维量（需要画辅助线/列表分析/分类讨论）
    - 不是纯计算，而是要展现思维过程
...
1. 所有数值必须合理：
   - 答案必须是整数或有限小数或简单常见分数（1/2, 1/3, 3/4等）
   - 禁止出现答案为π的无理数近似（除非题目明确要求保留π）
   
2. 干扰项设计原则：
   - 选择题B/C选项必须是"做了这题但犯典型错"的结果
   - 不能出现"一眼就排除"的弱干扰项
...
5. 场景现代化：
   - 用2025-2026年的真实场景（网购/快递/共享单车/环保/AI相关）
   - 禁止使用以下陈旧场景：
     ✗ "水池注水放水" → 改为 "打印机打印文件"
     ✗ "甲乙两人从A地到B地" → 改为具体人物和地点
     ✗ "商店里买铅笔橡皮" → 改为网购/点外卖等现代场景
...
共18道题，合计100分。
选择题answer为正确选项字母"A"/"B"/"C"/"D"。
填空题和解答题answer为最终答案字符串（数值或简短文字）。
所有题目手机端可输入答案。
